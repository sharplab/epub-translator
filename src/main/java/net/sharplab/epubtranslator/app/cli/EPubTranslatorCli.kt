...
class EPubTranslatorCli(private val ePubTranslatorAppService: EPubTranslatorAppService, private val ePubTranslatorSetting: EPubTranslatorSetting) : Runnable {
    ...
    override fun run() {
        val srcFile = src
        if(srcFile == null){
            throw IllegalArgumentException("src must not be null")
        }
        val resolvedSrcLang = srcLang ?: ePubTranslatorSetting.defaultSrcLang
        val resolvedDstLang = dstLang ?: ePubTranslatorSetting.defaultDstLang
    ...
    private fun constructDstFileFromSrcFile(src: File, dstLang: String?): File {
        ...
        dstFileName = when {
            !srcFileName.contains(".") -> "$srcFileName.$dstLang"
            else -> srcFileName.substring(0, srcFileName.lastIndexOf(".")) + "." + dstLang + srcFileName.substring(srcFileName.lastIndexOf("."), srcFileName.length)
        }
